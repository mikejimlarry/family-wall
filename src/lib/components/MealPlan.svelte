...
<div class="flex flex-col h-full gap-3">
	...
	<div class="flex items-center justify-between">
		...
		<div class="flex items-center gap-1">
			<button
				onclick={() => weekOffset--}
				class="w-10 h-10 flex items-center justify-center rounded-full text-slate-400 hover:text-white hover:bg-slate-700 transition-colors text-xl"
			>‹</button>
			<button
				onclick={() => weekOffset = 0}
				class="text-sm px-3 py-2 rounded-full transition-colors {weekOffset === 0
	...
	<div class="flex flex-col gap-1 overflow-y-auto flex-1 pr-1">
		{#each weekDays as day (day.date)}
			...
			<div class="rounded-xl overflow-hidden border border-slate-800 {day.isToday ? 'ring-1 ring-blue-500/50' : ''}">
				{#if isEditing}
					<!-- Edit form -->
					<div class="bg-slate-800 p-3 flex flex-col gap-2">
						<div class="flex items-center gap-2 mb-1">
							<span class="text-sm font-semibold text-slate-400">{day.dayName} {day.month} {day.dayNum}</span>
							<button onclick={cancelEdit} class="ml-auto text-slate-400 hover:text-slate-200 text-sm px-2 py-1.5">✕ Cancel</button>
						</div>
						...
						<input
							...
							autofocus
							class="bg-slate-700 border border-slate-600 rounded-lg px-3 py-3 text-base text-white placeholder-slate-500 outline-none focus:ring-1 focus:ring-blue-500"
						/>
						<input
							type="text"
						...
						<div class="flex gap-2">
							{#if meal}
								<button
									...
									class="py-3 px-4 rounded-lg bg-slate-700 hover:bg-red-900/40 text-red-400 text-sm font-medium transition-colors"
								>Remove</button>
							{/if}
							<button
								onclick={submitEdit}
								disabled={!editValue.trim()}
								class="flex-1 py-3 rounded-lg bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white text-base font-semibold transition-colors"
							>Save</button>
						</div>
					</div>
				{:else}
					...
					<button
						...
						disabled={!adminMode}
						class="w-full flex items-center gap-3 px-3 py-3 text-left transition-colors {meal
							? 'bg-slate-800/80 hover:bg-slate-800'
							: 'bg-slate-900/40 hover:bg-slate-800/40'} {day.isToday ? 'bg-slate-800' : ''} {adminMode ? 'cursor-pointer' : 'cursor-default'}"
					>
						<!-- Day label -->
						<div class="shrink-0 w-14">
							<p class="text-sm font-semibold {day.isToday ? 'text-blue-400' : 'text-slate-500'}">{day.dayName}</p>
						...
						<div class="flex-1 min-w-0">
							{#if meal}
								<p class="text-base text-slate-200 truncate">{meal.meal}</p>
								{#if meal.notes}
									<p class="text-xs text-slate-500 truncate">{meal.notes}</p>
								{/if}
							{:else}
								<p class="text-base text-slate-700 italic">{adminMode ? 'Tap to plan…' : '—'}</p>
							{/if}
						</div>
